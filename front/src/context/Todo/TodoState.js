...
const TodoState = (props)=>{    
    ...
    const createTodo = async(title, color)=>{
        ...
        const res = await axios.post(`http://localhost:5000/api/createTodo`,{
            title,
            color
        },
        {
    ...
    const deleteTodo = async(todoId)=>{
        const res = await axios.delete(`http://localhost:5000/api/deleteTodo/${todoId}`,{
            headers
        });
        console.log(res)
    ...
    const editTodo = async(todoId,editedPart)=>{
        const res = await axios.put(`http://localhost:5000/api/editTodo/${todoId}`,editedPart,{
            headers
        });
