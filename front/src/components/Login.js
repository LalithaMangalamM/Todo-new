...
const Login = () => {
    ...
    const handleSubmit = async(e)=>{
        e.preventDefault();
            ...
            try {
                const response = await axios.post(`http://localhost:5000/v1/u/login`, {
                  email,
                  password
                });
                ...
                if (success) {
                  // Set the 'token' cookie with the received value
                  setCookie('token', token, { path: '/', maxAge: 3600 }); // Adjust maxAge as needed
